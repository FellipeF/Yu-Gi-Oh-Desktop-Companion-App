...
def _column_exists(cursor, table_name: str, column_name: str) -> bool:
    """Checks if the column already exists for newly created versions"""
    cursor.execute(f"PRAGMA table_info({table_name})")
    columns = cursor.fetchall()
    return any(column[1] == column_name for column in columns)

def _migrate_user_decks_notes(cursor):
    """Adding the column NOTES on an user deck"""
    if _column_exists(cursor, "user_decks", "notes"):
        return

    cursor.execute("ALTER TABLE user_decks ADD COLUMN notes TEXT NOT NULL DEFAULT ''")
...
def run_migrations():
    ...
    try:
        _migrate_user_decks_notes(cursor)
        conn.commit()
